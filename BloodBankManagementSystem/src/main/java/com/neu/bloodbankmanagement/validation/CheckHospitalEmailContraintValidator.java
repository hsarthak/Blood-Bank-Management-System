...
public class CheckHospitalEmailContraintValidator implements ConstraintValidator<CheckHospitalEmail, String>{
	...
	public boolean isValid(String userEnteredValue, ConstraintValidatorContext context) {
		...
		try {
			...
			for(Hospital hospital: hospitals) {
				if(hospital.getEmail().equalsIgnoreCase(userEnteredValue)) {
					result = false ;
					break;
				}else {
					result = true;
