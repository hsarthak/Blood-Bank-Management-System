...
public class CheckDonorEmailContraintValidator implements ConstraintValidator<CheckDonorEmail, String>{
	...
	public boolean isValid(String userEnteredValue, ConstraintValidatorContext context) {
		...
		try {
			...
			for(Donor donor: donors) {
				if(donor.getEmail().equalsIgnoreCase(userEnteredValue)) {
					result = false ;
					break;
				}else {
					result = true;
